...
echo "[1/2] Measuring Phi-3.5 Mini (Q4_K_M) J/token across threads: $THREAD_SETS"
for T in $THREAD_SETS; do
  ...
  THREADS=$T PROMPT_TOKENS=$PROMPT_TOKENS GEN_TOKENS=$GEN_TOKENS IDLE_SECS=$IDLE_SECS ACTIVE_SECS=$ACTIVE_SECS \
    bash "$ROOT/scripts/measure_phi_jpt.sh"
done

echo "[2/2] Building Phi benchmark report..."
python3 "$ROOT/scripts/build_phi_report.py"

echo "Done. Reports written to:"
echo "  - $ROOT/PHI_BENCHMARK.md"
echo "  - $ROOT/benchmarks/phi_full_report.md"
