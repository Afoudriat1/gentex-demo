...
# Measure Gemma 2 2B Instruct (Q4_K_M) energy per token on Apple Silicon.

MODEL_PATH="/Users/andrewfoudriat/GENTEX DEMO/models/gemma-2-2b-it-Q4_K_M.gguf"
...
IDLE_OUT="/tmp/power_idle_gemma.txt"
ACTIVE_OUT="/tmp/power_active_gemma.txt"
BENCH_OUT="/tmp/llama_bench_gemma.txt"

BENCH_DIR="/Users/andrewfoudriat/GENTEX DEMO/benchmarks"
SUMMARY_CSV="$BENCH_DIR/gemma_metrics.csv"
SUMMARY_JSONL="$BENCH_DIR/gemma_metrics.jsonl"
REPORT_MD="/Users/andrewfoudriat/GENTEX DEMO/GEMMA_BENCHMARK.md"
...
if [[ ! -f "$MODEL_PATH" ]]; then
  echo "ERROR: Gemma model not found at $MODEL_PATH" >&2
  exit 1
fi
...
echo "Running llama-bench (Gemma 2 2B Q4_K_M, -ngl $NGL)..."
...
# Split on '|' and take the last non-empty field (typically t/s like '21.17 ± 0.41').
...
echo ""
echo "===== Gemma 2 2B Q4_K_M – J/token Estimate ====="
