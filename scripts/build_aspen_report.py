...
CSV_PATH = BENCH_DIR / 'aspen_metrics.csv'
JSONL_PATH = BENCH_DIR / 'aspen_metrics.jsonl'
MODEL_PATH = Path('/Users/andrewfoudriat/MODEL DEMO/server/aspen-4b.gguf')
REPORT_HTML = BENCH_DIR / 'aspen_full_report.html'
REPORT_MD = BENCH_DIR / 'aspen_full_report.md'
...
def build_markdown(latest, rows):
    ...
    # Known-good memory breakdown for Aspen 4B (ctx=2048, CPU-only) from llama-server logs
    model_buf_mib = 1035.93
    kv_cache_mib = 288.00
    compute_buf_mib = 301.75
    output_buf_mib = 0.58
    ...
    return (
        f"# Aspen 4B – Full Benchmark Report\n\n"
        ...
        f"- Runtime memory breakdown (ctx=2048, CPU-only):\n"
        ...
        f"Notes: KV cache grows roughly linearly with context size; halving ctx to 1024 reduces KV to ~144 MiB.\n\n"
