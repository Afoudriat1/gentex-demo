...
# Measure Aspen 4B energy per token (J/token) on Apple Silicon
...
MODEL_PATH="/Users/andrewfoudriat/MODEL DEMO/server/aspen-4b.gguf"
...
BENCH_OUT="/tmp/llama_bench_aspen.txt"

# Persisted summaries
BENCH_DIR="/Users/andrewfoudriat/GENTEX DEMO/benchmarks"
SUMMARY_CSV="$BENCH_DIR/aspen_metrics.csv"
SUMMARY_JSONL="$BENCH_DIR/aspen_metrics.jsonl"
REPORT_MD="/Users/andrewfoudriat/GENTEX DEMO/ASPEN_BENCHMARK.md"
...
if [[ ! -f "$MODEL_PATH" ]]; then
  echo "ERROR: Aspen model not found at $MODEL_PATH" >&2
  exit 1
fi
...
echo "Running llama-bench (Aspen CPU-only) ..."
