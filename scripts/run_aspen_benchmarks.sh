...
echo "[1/2] Measuring Aspen J/token and throughput across threads: $THREAD_SETS"
for T in $THREAD_SETS; do
  ...
  THREADS=$T PROMPT_TOKENS=$PROMPT_TOKENS GEN_TOKENS=$GEN_TOKENS IDLE_SECS=$IDLE_SECS ACTIVE_SECS=$ACTIVE_SECS \
    bash "$ROOT/scripts/measure_aspen_jpt.sh"
done

echo "[2/2] Building full Markdown/HTML report..."
python3 "$ROOT/scripts/build_aspen_report.py"

echo "Done. Reports at:"
echo "  - $ROOT/ASPEN_BENCHMARK.md (latest human-readable summary)"
echo "  - $ROOT/benchmarks/aspen_full_report.md"
echo "  - $ROOT/benchmarks/aspen_full_report.html"
