...
# Measure Phi-3.5 Mini (3.8B, Q4_K_M) energy per token on Apple Silicon.

MODEL_PATH="/Users/andrewfoudriat/GENTEX DEMO/models/Phi-3.5-mini-instruct-Q4_K_M.gguf"
...
IDLE_OUT="/tmp/power_idle_phi.txt"
ACTIVE_OUT="/tmp/power_active_phi.txt"
BENCH_OUT="/tmp/llama_bench_phi.txt"

BENCH_DIR="/Users/andrewfoudriat/GENTEX DEMO/benchmarks"
SUMMARY_CSV="$BENCH_DIR/phi_metrics.csv"
SUMMARY_JSONL="$BENCH_DIR/phi_metrics.jsonl"
REPORT_MD="/Users/andrewfoudriat/GENTEX DEMO/PHI_BENCHMARK.md"
...
if [[ ! -f "$MODEL_PATH" ]]; then
  echo "ERROR: Phi-3.5 model not found at $MODEL_PATH" >&2
  exit 1
fi
...
echo "Running llama-bench (Phi-3.5 Mini Q4_K_M, -ngl $NGL)..."
...
# In llama-bench output, the t/s value is in the last column.
# Split on '|' and take the last non-empty field (e.g. '21.17 ± 0.41').
...
echo ""
echo "===== Phi-3.5 Mini Q4_K_M – J/token Estimate ====="
